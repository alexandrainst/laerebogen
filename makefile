...
dataset: ## Generate the dataset
	...
	@uv run src/scripts/generate_base_dataset.py && \
		uv run src/scripts/push_to_hub.py data/dataset.jsonl saattrupdan/laerebogen::base && \
		uv run src/scripts/correct_grammar_in_dataset.py && \
		uv run src/scripts/push_to_hub.py data/dataset.grammar_corrected.jsonl saattrupdan/laerebogen::grammar_corrected && \
		uv run src/scripts/correct_quality_in_dataset.py && \
		uv run src/scripts/push_to_hub.py data/dataset.quality_corrected.jsonl saattrupdan/laerebogen::quality_corrected && \
		uv run src/scripts/evolve_dataset.py && \
		uv run src/scripts/push_to_hub.py data/dataset.evolved.jsonl saattrupdan/laerebogen::evolved && \
		uv run src/scripts/add_follow_ups.py && \
		uv run src/scripts/push_to_hub.py data/dataset.follow_ups_added.jsonl saattrupdan/laerebogen::with_follow_ups --default
